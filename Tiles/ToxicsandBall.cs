...
{
    class ToxicsandBall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 277 , 0f, 0f, 0);
        ...
        public override void Kill(int timeLeft)
        {
            if (projectile.owner == Main.myPlayer && !projectile.noDropItem)
            {
                ...
                if (!Main.tile[tileX, tileY].active())
                {
                    ...
                    if (!onMinecartTrack)
                    {
                        WorldGen.PlaceTile(tileX, tileY, mod.TileType<Toxicsand>(), false, true, -1, 0);
                    }
                    if (!onMinecartTrack && Main.tile[tileX, tileY].active() && Main.tile[tileX, tileY].type == mod.TileType<Toxicsand>())
                    {
                        ...
                        if (Main.netMode != 0)
                        {
                            NetMessage.SendData(17, -1, -1, null, 1, tileX, tileY, mod.TileType<Toxicsand>(), 0, 0, 0);
                        }
                    }
                }
            }
